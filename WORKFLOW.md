...
This is the channel copy of the canonical scriptwriting workflow, aligned on 2026-09-22.
Maintain shared contract changes in scriptwriting and synchronize this copy. The Agent directory
below is the single mapping used by this checkout's AGENTS.md and SKILLS.md.
Multica runs each agent with its mapped installed Hermes profile against this content repository.
An already assigned scripting agent performs its own role here; the profile-source checkout is
not a runtime dependency. Do not launch a second profile to perform your own assigned work.

In this document, active `templates/issue.md` and `templates/episode-index.md` references mean
files under the executing profile's `$HERMES_HOME/templates/`, not legacy channel copies.
`scripts/profiles.json` and installer commands describe tooling in the scriptwriting source repo;
they are not required files or commands for stage workers in this content checkout.
The deployment record below retains unresolved pilot gates; this local alignment is not a live
cutover or evidence that existing issues satisfy the new contract.
