...
## Channel copy provenance

Copied from the scriptwriting source repository on 2026-09-22 for instruction alignment.
The evidence below records that source migration's observations, not a fresh live pilot of this
channel. Preserve its unresolved gates. The installed Architect was separately observed to carry
`multica-pr-v1` instructions during diagnosis; that does not establish a coherent six-agent rollout.
The source migration plan remains in the scriptwriting repository. Do not require that repository
to be checked out by a stage agent; Head owns deployment reconciliation.
...
`docs/superpowers/plans/2026-09-22-multica-issue-pr-workflow.md` in the scriptwriting source repository.
